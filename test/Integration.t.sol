...
contract IntegrationTest is Test {
  ...
  function test_shouldSwapUsingFlashLoan() public {
    GasProviderHelper gasProviderHelper = new GasProviderHelper(0x92f1C3d951018C90C364c234ff5fEE00f334072F, address(usdc));
    (uint256 ethToSend, bytes memory swapCalldata) = getSwapCalldata();
    uint256 balanceBefore = address(this).balance;

    uint256 balanceAfter = gasProviderHelper.swapWithFlashloan(
    ...
    console2.log("Delta balance is: %s", balanceAfter - balanceBefore);
    assertEq(profitInWei, balanceAfter - balanceBefore);
  }

  function getSwapCalldata() private returns (uint256 ethToSend, bytes memory swapCalldata) {
    ...
    // prepare signature for reward
    (uint8 rewardV, bytes32 rewardR, bytes32 rewardS) = getRewardSignature(reward, permitHash);
    ethToSend = gasBroker.getEthAmount(address(usdc), value - reward);

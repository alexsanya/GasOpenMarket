...
import gasProviderHelperArtifact from "../out/GasProviderHelper.sol/GasProviderHelper.json" assert { type: 'json' };
...
const GAS_BROKER_ADDRESS = "0x92f1C3d951018C90C364c234ff5fEE00f334072F";
...
async function setUp() {
  ...
  const gasProviderHelper = await deployContract('GasProviderHelper', gasProviderHelperArtifact, GAS_BROKER_ADDRESS, USDC);
  const fundUSDCcontract = await deployContract('FundUSDC', fundUSDCArtifact);
  
  //fund wallet with USDC
  let tx = await fundUSDCcontract.swapExactOutputSingle(100n*10n**6n, 300n*ONE_ETH, { value: 300n*ONE_ETH });
  await tx.wait();

  console.log(`Deployer USDC ballance is ${await usdcContract.balanceOf(signer.address)}`);
  tx = await usdcContract.connect(signer).transfer(CUSTOMER_ADDRESS, 100n*10n**6n);
  await tx.wait();
